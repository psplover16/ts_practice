...
// 物件的迭代器（沒有內建）
const obj = { a: 1, b: 2 };
console.log("物件的迭代器:", (obj as any)[Symbol.iterator]); // undefined // 告訴 TypeScript「別管型別了，當作任何型別都可以」
...
class NumberRange {
    ...
    [Symbol.iterator]() {
        ...
        // 回傳迭代器物件，必須有 next() 方法
        return {
            next(): { value: number; done: false } | { value: undefined; done: true } {
                if (current <= end) {
                    // 還有值要提供：回傳當前值並遞增
...
class GradeBook {
    ...
    [Symbol.iterator]() {
        ...
        return {
            next(): { value: Student; done: false } | { value: undefined; done: true } {
...
// 導出類別和型別供其他檔案使用
export { NumberRange, GradeBook, FibonacciSequence, TaskQueue };
export type { Student, Task };
