...
// ✅ 正確：接受 Boolean 物件型別
let createdByNewBoolean2: Boolean = new Boolean(1);  // 這是物件
// ✅ 正確：Boolean() 函式轉換，回傳原始 boolean
let createdByBoolean: boolean = Boolean(1);  // 這是原始型別
...
let hexValue: number = 0xff;  // 十六進位
let binaryValue: number = 0b101;  // 二進位
let octalValue: number = 0o755;  // 八進位
...
let message: string = '歡迎使用 TypeScript';
let template: string = `Hello, ${userName}!,你是否為${age+1}歲??`;  // 模板字串
...
let data: null = null;
let userInfo: string | null = null;  // 可能是字串或null
// undefined 通常表示變數尚未被賦值
let result: undefined = undefined;
let optionalValue: number | undefined = undefined;  // 可能是數字或undefined
...
// 7. Symbol (ES6新型別)
let id: symbol = Symbol('a');
let uniqueKey: symbol = Symbol('a');
...
let descriptionId: symbol = Symbol.for('a');
let descriptionUniqueKey: symbol = Symbol.for('a');
