...
let mixedData = [true, 42, "TypeScript", false];
...
console.log("\n6. 元組 (Tuples)：");
// 元組 - 每個位置都有固定的型別
let person = ["張三", 25, true];
let coordinate = [10.5, 20.3];
let rgbColor = [255, 128, 0];
...
// 可選元組元素
let optionalTuple = ["測試", 123]; // 第三個元素可選
console.log("可選元組:", optionalTuple);
// ============================================
// 7. 陣列方法與型別安全
// ============================================
console.log("\n7. 陣列方法與型別安全：");
let productPrices = [100, 250, 80, 150, 300];
// map - 轉換每個元素，保持型別安全
let discountedPrices = productPrices.map(price => price * 0.8);
console.log("原價:", productPrices);
console.log("八折價:", discountedPrices);
// filter - 過濾元素，保持型別
let expensiveProducts = productPrices.filter(price => price > 200);
console.log("高價商品 (>200):", expensiveProducts);
// reduce - 累積計算
let totalPrice = productPrices.reduce((sum, price) => sum + price, 0);
...
let foundPrice = productPrices.find(price => price === 150);
